...
use zkv_para_evm_client_evm_tracing::{formatters::ResponseFormatter, types::single};
pub use zkv_para_evm_rpc_core_debug::{DebugServer, TraceParams};
use zkv_para_evm_rpc_core_types::{RequestBlockId, RequestBlockTag};
use zkv_para_evm_rpc_primitives_debug::{DebugRuntimeApi, TracerInput};
            ...
            single::TraceType::CallList => {
                ...
                let response = match tracer_input {
                    TracerInput::CallTracer => {
                        zkv_para_evm_client_evm_tracing::formatters::CallTracer::format(proxy)
            ...
            if let Some(transaction) = transactions.get(index) {
                let f = || -> RpcResult<_> {
                    ...
                    if trace_api_version == 1 {
                        ...
                        //Should never happen
                        return Err(internal_err(format!(
                            "Wrong trace_api_version, expected 1 found: {:?}",
                            trace_api_version
                        )));
                    }

                    Ok(zkv_para_evm_rpc_primitives_debug::Response::Single)
                };
                ...
                return match trace_type {
                    single::TraceType::Raw {
                        ...
                        let mut proxy = zkv_para_evm_client_evm_tracing::listeners::Raw::new(
                            disable_storage,
                            disable_memory,
                            disable_stack,
                            raw_max_memory_usage,
                        ...
                        proxy.using(f)?;
                        Ok(Response::Single(
                            zkv_para_evm_client_evm_tracing::formatters::Raw::format(proxy).ok_or(
                                internal_err(
                                    "replayed transaction generated too much data. try disabling \
									 memory or storage?",
                                ),
                            )?,
                        ))
                    ...
                    single::TraceType::CallList => {
                        let mut proxy =
                            zkv_para_evm_client_evm_tracing::listeners::CallList::default();
                        proxy.using(f)?;
                        proxy.finish_transaction();
                        let response = match tracer_input {
                            TracerInput::Blockscout => {
                                zkv_para_evm_client_evm_tracing::formatters::Blockscout::format(
                                    proxy,
                                )
                                .ok_or("Trace result is empty.")
                                .map_err(|e| internal_err(format!("{:?}", e)))
                            ...
                            TracerInput::CallTracer => {
                                let mut res =
                                    zkv_para_evm_client_evm_tracing::formatters::CallTracer::format(
                                        proxy,
                                    )
                                    .ok_or("Trace result is empty.")
                                    .map_err(|e| internal_err(format!("{:?}", e)))?;
